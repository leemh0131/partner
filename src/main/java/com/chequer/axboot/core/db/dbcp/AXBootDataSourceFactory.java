...
public class AXBootDataSourceFactory {
    ...
    public static AXBootDBCP2DataSource create(String dataSourceId, AXBootContextConfig.DataSourceConfig2 dataSourceConfig) throws Exception {
        try {
            AXBootDBCP2DataSource axBootDBCP2DataSource = new ModelMapper().map(dataSourceConfig, AXBootDBCP2DataSource.class);
            axBootDBCP2DataSource.setDatabaseType(dataSourceConfig.getHibernateConfig().getDatabaseType());
            Connection conn = axBootDBCP2DataSource.getConnection();
    ...
    //DataSource 추가 20210329 KHJ
    public static AXBootDBCP2DataSource create(AXBootContextConfig.DataSourceConfig2 dataSourceConfig) throws Exception {
        return create("MsDataSource", dataSourceConfig);
    }
}
