    ...
    function html5Upload() {
    	var tempFile,
    		sUploadURL;
		sUploadURL = '/api/SPD/APPROVE00006/naverSmartEditorMultiImageUpload';
    ...
    function callAjaxForHTML5 (tempFile, sUploadURL){
		...
		$.ajax({
			...
			success: function (res) {
				if(res.map.state == "noImage"){
					var sFileName = res.map.msg.replace("NOTALLOW_", "");
					alert("이미지 파일(jpg,gif,png,bmp)만 업로드 하실 수 있습니다. ("+sFileName+")");
				} else {
					//성공 시에  responseText를 가지고 array로 만드는 부분.
					makeArrayFromString(res.map.msg);
				}
	 ...
	 */
 	function setPhotoToEditor(oFileInfo){
		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			...
			oFileInfo[0].sFileURL = "http://" + window.location.hostname + ":" + window.location.port + "/file/" + oFileInfo[0].sFileURL;
