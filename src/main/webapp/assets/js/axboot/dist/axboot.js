        ...
        if (inputs.length > 0) {
            for (var i = 0; i < inputs.length; i++) {
                ...
                if (nvl(inputs[i].getAttribute('formatter')) != '') {
                    ...
                    if (mode.replace(/-/g, '').toUpperCase() == 'YYYYMMDD') {
                        ...
                        target.on("keypress", function(event){
                            ...
                            this.value = value.replace(/[\ㄱ-ㅎㅏ-ㅣ가-힣]/g, '');
                        })
                    }
                    if (mode.replace(/-/g, '').toUpperCase() == 'YYYYMM') {
                        target[0].maxLength = 7;
                        target.val(target.val().replace(/(\d{4})(\d{2})/, '$1-$2'));
                    ...
                    if (mode == 'persent') {
                        target.on('keypress', function (event) {
                            ...
                            var _pattern1 = /^\d{11}$/;
                            //현재 value값이 11자리 숫자이면 . 만 입력가능  {숫자}의 값을 변경하면 자리수를 조정할 수 있다.
                            ...
                            //소수점 둘째자리까지만 입력가능
                            var _pattern2 = /^\d*[.]\d{2}$/;
                            //현재 value값이 소수점 둘째짜리 숫자이면 더이상 입력 불가  {숫자}의 값을 변경하면 자리수를 조정할 수 있다.
                            ...
                            if (num.indexOf('.') > -1) {
                                ...
                                var num_back = num.split('.')[1];
                                var num_front = num.split('.')[0];

                                if (num_back.length == 1) {
                                    num_back = num_back + "0";
                                } else if (num_back.length == 0) {
                                    num_back = "00";
                                }

                                this.value = num_front + "." + num_back;
                            } else {
                                this.value = num + ".00";
                            }
                        });
                    ...
                    if (mode == 'rt_exch') {
                        if (nvl(target.val()) == '') {
                            ...
                            target.val(Number(Number(target.val().replace(/[^-\.0-9]/g, '')).toFixed(2)))
                        }
                        ...
                        target.on('keypress', function (event) {
                            ...
                            if (num.indexOf('.') > -1) {
                                ...
                                num_front = num_front + "";
                                point = num_front.length % 3;
                                len = num_front.length;

                                str = num_front.substring(0, point);
                                while (point < len) {
                                    if (str != "") str += ",";
                                    str += num_front.substring(point, point + 3);
                                    point += 3;
                                }
                                this.value = str + "." + num_back;
                            } else {
                                num = num.replace(/-/g, "");
                                num = num + "";
                                point = num.length % 3;
                    ...
                    if (mode == 'money') {
                        ...
                        target.on('keypress', function (event) {
                            ...
                            var _value = this.value.replace(/[\ㄱ-ㅎㅏ-ㅣ가-힣]/g, '');
...
axboot.ajax = function () {
    ...
    return function (http) {
        ...
        httpOpts = {
            contentType: (http.contentType == null || http.contentType == undefined) ? options.contentType : http.contentType,
        };
