...
<ax:set key="title" value="공통코드 검색 도움창"/>
...
<ax:layout name="base">
    <jsp:attribute name="script">
        ...
        <script type="text/javascript">
			...
			if(initData && initData.initData && initData.initData.TITLE){
				title = initData.initData.TITLE;
				$(".title").html('<i class="cqc-browser"></i>' + title + '검색 도움창</h1>');
				$(".listName").html('<i class="cqc-list"></i>' + title + '리스트');
			}
            ...
            var ACTIONS = axboot.actionExtend(fnObj, {
                ...
                PAGE_SEARCH: function (caller, act, data) {
                    ...
                    var data = $.DATA_SEARCH("commonHelp", ACTION, $.extend(initData['initData'], {KEYWORD: $("#KEYWORD").val()}), fnObj.gridView01).list;

                    var chkArr = [];
                    var temp2 = [];
                    //선택데이터세팅
                    for (var i = 0; i < temp.length; i++) {
                        chkArr.push(temp[i].SYSDEF_CD);
                    }
                    chkArr = chkArr.join('|');

                    if(temp.length > 0){
                        for (var i = 0; i < data.length; i++) {
                            if (chkArr.indexOf(data[i].SYSDEF_CD) == -1) {
                                temp2.push(data[i]);
                            }
                        }
                ...
                initView: function () {
                    ...
                    this.target = axboot.gridBuilder({
                        ...
                        columns: [
                            ...
                            {key: "SYSDEF_CD", label: title + "코드", width: 100, align: "center", editor: false, sortable: true},
                            {key: "SYSDEF_NM", label: title + "명", width: "*", align: "left", editor: false, sortable: true}
                        ],
                        body: {
                            onClick: function () {
